export default class MenuScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // Load 7 symbols
    for (let i = 1; i <= 7; i++) {
      this.load.image(`SEMBOL${i}`, `${baseUrl}assets/SEMBOL${i}.png`);
    }
    // Load 7 digit cards (1-7)
    for (let i = 1; i <= 7; i++) {
      this.load.image(`digit${i}`, `${baseUrl}assets/${i}.png`);
    }

    // UI Assets
    this.load.image('background', `${baseUrl}assets/ARKAPLAN.png`);
    this.load.image('header', `${baseUrl}assets/HEADER.png`);
    this.load.image('footer', `${baseUrl}assets/FOOTER.png`);
    this.load.image('refKey', `${baseUrl}assets/REFERANSANAHTARI.png`);
    this.load.image('scores', `${baseUrl}assets/SCORES&TIMEBAR.png`);
  }
  ...
  create() {
    ...
    form.on('click', (e) => {
      if (e.target && e.target.id === 'startBtn') {
        ...
        // clean up DOM and start info scene
        try { form.removeListener('click'); } catch (_) { }
        try { form.destroy(); } catch (_) { }
        this.scene.start('InfoScene', { playerName, companyName });
      }
    });
  }
